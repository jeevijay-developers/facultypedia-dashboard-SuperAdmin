		...
		Object.entries(params).forEach(([key, value]) => {
			...
			if (Array.isArray(value)) {
				value
					...
					.forEach((entry) => url.searchParams.append(key, safeToString(entry)));
				return;
			}
...
export const getCachedAdminProfile = () => safeJSONParse(storage.getItem(PROFILE_KEY));
...
const hasAutoAuthCredentials = Boolean(configuredAdminEmail) && Boolean(configuredAdminPassword);
